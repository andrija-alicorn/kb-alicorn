    ...
    <div class="card-body">
        <form action="{{ route("admin.articles.update", [$article->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('categories') ? 'has-error' : '' }}">
                <label class="category-label">{{ trans('cruds.article.fields.categories') }}</label>
                    @foreach($parents as $parent)
                        <div class="parent-category" >
                            <input type="checkbox" id="category-{{$parent->id}}" multiple="multiple" name="categories[]" value="{{ $parent->id }}" {{ (in_array($parent->id, old('categories', [])) || isset($article) && $article->categories->contains($parent->id)) ? 'checked' : '' }} />
                            <label for="category-{{$parent->id}}">{{$parent->name}}</label>
                        </div>
                         @foreach($parent->children as $child)
                            <div class="child-category">
                                <input type="checkbox" id="category-{{$child->id}}" multiple="multiple" name="categories[]" value="{{ $child->id }}" {{ (in_array($child->id, old('categories', [])) || isset($article) && $article->categories->contains($child->id)) ? 'checked' : '' }} />
